...
function get_jsapi_parameters($data)
{
    ...
    $unifiedOrder->setParameter("total_fee", intval($price * 100));
//    SITE_URL
    // C('HTTP_HOST') . '/Data/notify.php'
    // 转发微信支付消息
    $unifiedOrder->setParameter("notify_url", SITE_BASE_URL . 'Data/notify.php');
...
//    notify
    ...
    $unifiedOrder->setParameter("attach", json_encode($order_data));//附加数据
 ...
 * 查询支付详情
 */
function wechat_query_order($transaction_id,$mpid=null)
{
    wechat_pay_config($mpid);
    vendor('WechatPay.Wechatpay');
    $input = new PayNotifyCallBack();
    $result = $input->Queryorder($transaction_id);
 ...
 * 微信支付配置
 */
function wechat_pay_config($mpid=null)
{
    if(!empty($mpid)){
        $mpid = $mpid;
    }else{
    ...
    $appsecret = M('mp_setting')->where(array('mpid' => $mpid, 'name' => 'appsecret'))->getField('value');
    $mchid = M('mp_setting')->where(array('mpid' => $mpid, 'name' => 'mchid'))->getField('value');
    $paysignkey = M('mp_setting')->where(array('mpid' => $mpid, 'name' => 'paysignkey'))->getField('value');
    vendor('WechatPay.config');
    $config_arr = array(
        'appid' => $appid,
        'appsecret' => $appsecret,
        'mchid' => $mchid,
    ...
    new we_config($config_arr);
}

/*
 * 支付
 * 交易名 交易金额 回调网址 订单号 用户openid mpid
 * 返回支付参数
 */

//$name,$total_fee,$notify_url,$order_id,$openid,$mpid=null
function wechat_pay($params = array())
{
    wechat_pay_config();
    vendor('WechatPay.Wechatpay');
    $we = new Wechatpay();
    return $s2 = $we->wechat_pay($params);
}
